...
var Color = Base.extend(new function() {
        ...
        if (match) {
            ...
            if (!color) {
                if (window) {
                    ...
                    // -> 'colourName-rgb' is a ugly workaround for this problem
                    color = converters['colourName-rgb'](string) || [0, 0, 0];
                }
            }
    ...
    var converters = {
        ...
        'colourName-rgb': function(colour){
            // This is a workaround to convert color names properly when using paper-jsdom (and possibly web-workers)
            // Why? Color names (e.g. 'red' or 'cyan') are converted via canvas which are not present in paper-jsdom or web-workers
            // More info here https://github.com/paperjs/paper.js/issues/1901
            ...
            var hex = colourNameToHex(colour);
            if (!hex) { return false };
